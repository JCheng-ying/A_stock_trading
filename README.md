...
4. 重点关注（这一步也已用多进程并发刷新，观察池股票数越多提速越明显）：第一个
...
5. 结合股票所属板块的当日热门度（涨停股池接口自带"所属行业"，另外还会尝试标注
   更精确的板块涨跌幅）。
...
3. 此前30个交易日平均换手率 < 2%（`VOLUME_SURGE_LOOKBACK_DAYS` +
   `VOLUME_SURGE_AVG_TURNOVER_MAX_PCT`），说明此前确实是地量；
4. 当日换手率 >= 前30日平均换手率的2倍以上（`VOLUME_SURGE_RATIO`），说明突然放量；
5. 当日收盘价 < 前30日平均价格的1.2倍（`VOLUME_SURGE_PRICE_MAX_RATIO`），说明价格
   还没涨出去太多；
6. 是放量的**第一天**：过去30个交易日内，没有哪天也满足过第4条——避免同一轮放量
...
不过股价/市净率反推出来的每股净资产终归是个近似值。`check_book_value.py` 是一个
单独的、不用每天跑的脚本，查的是财报里披露的真实每股净资产（`stock_financial_
abstract` 接口，按单只股票查，跟实时快照走的是不同数据源，实测东方财富快照连不上
时这个接口反而还能用）。因为净资产按季度财报披露，不会天天变，查到过的股票会缓存
进 `book_value_cache` 表长期复用，默认只查"还没查过的"：

```bash
python check_book_value.py              # 只查两个股票池里还没查过净资产的股票
python check_book_value.py --force       # 新一季财报出来后，全部重新查一遍
python check_book_value.py --workers 16  # 调整并发进程数（默认8）
```

跑完会用真实值把两个股票池里净资产不达标的信号直接删掉（比如查出来"每股净资产
0.03元"甚至负数这种明显不该出现在结果里的），跑完记得 `python generate_site.py`
重新生成网页。`daily_scan.py` 每天扫描时，净资产过滤会优先用这份真实值缓存，查
不到的股票才退回股价/市净率反推的近似值。
