...
def main():
    ...
    if not cap_filter_applied:
        ...
        print("    市值/净资产数据来自本次东方财富实时快照（已顺带缓存，供以后连不上时使用）。")
    elif cap_source == "live(腾讯逐只兜底)":
        print("    ⚠️ 东方财富本次连不上，改用腾讯行情接口逐只查市值兜底（只有市值，没有"
              "市净率近似值——净资产这条这次先不过滤，交给 check_book_value.py 事后用财报"
              "真实值核对删除）。")
    else:
